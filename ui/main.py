...
def index() -> None:
    ...
    with ui.card().style(
        ...
        ui.label("Welcome to SUNET Transcriber").classes("text-h5")
        ui.separator()
        ...
        ui.separator()
        with ui.row():
            ui.button("Login", icon="login").on("click", login)
            ui.button("Login with SSO", icon="login")


ui.run(storage_secret="secret", title="SUNET Transcriber", port=8888)
